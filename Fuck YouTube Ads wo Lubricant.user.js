...
// @license      MIT
// @downloadURL https://update.greasyfork.org/scripts/557720/Fuck%20YouTube%20Ads%20wo%20Lubricant.user.js
// @updateURL https://github.com/vippium/vBlockTube/raw/refs/heads/main/Fuck%20YouTube%20Ads%20wo%20Lubricant.user.js
// ==/UserScript==
...
(function () {
    ...
    function init() {
        ...
        // Global shorts blocker observer
                        ...
                        set: function (newValue) {
                            ...
                            if (newValue.set) {
                                ...
                                newValue.set = function () {
                                    // hook ytmusic initial data YTMUSIC_INITIAL_DATA
                    ...
                    const fetch_ = async function (uri, options) {
                        async function fetch_request(response) {
                            ...
                            if (url.includes('youtubei/v1/browse?prettyPrint=false')) {
                                ...
                                // Ignore music homepage and movie homepage
    ...
    function on_page_change() {
        ...
        const wait_configs = {
            ...
            "yt_home": [
                {
                    "seletor": "#contents ytd-rich-section-renderer yt-shelf-header-layout h2.yt-shelf-header-layout__title span.yt-core-attributed-string",
                    "inject": true,
        ...
        function set_mobile_shorts_download(wrapper_node) {
            ...
            if (nodes.length !== 2) {
                log('set_mobile_shorts_download wrong number of nodes', nodes.length, -1);
                return;
            }
        ...
        return {
            flag_infos: {
                "zh-CN": {
                    ...
                    "btn_sponsorblock_tips": "使用 SponsorBlock 接口自动跳过视频中的赞助内容"
                },
                "zh-TW": {
                    "sponsored": "贊助商廣告",
                    "free_movie": "免費（含廣告）",
                    ...
                    "btn_dbclick_download_video_tips": "雙擊按讚下載影片，雙擊不喜歡下載音檔",
                    ...
                    "btn_sponsorblock_tips": "使用 SponsorBlock API 自動跳過影片中的贊助內容"
                },
                "zh-HK": {
                    "sponsored": "贊助廣告",
                    "free_movie": "免費（有廣告）",
                    ...
                    "rule_info": "規則資訊",
                    "del_config_confirm_tips": "你確定要刪除所有設定資訊嗎？",
                    "btn_dbclick_download_video_tips": "雙擊喜歡下載影片，雙擊不喜歡下載音檔",
                    "btn_dbclick_download_video_title": "雙擊下載影片",
                    "download_video_confirm_tips": "要下載這個影片嗎？",
                    "download_audio_confirm_tips": "要下載這個音檔嗎？",
                    "btn_shorts_auto_scroll_title": "自動捲動",
                    ...
                    "btn_sponsorblock_tips": "使用 SponsorBlock API 自動略過影片中的贊助片段"
                },
                "en": {
                    ...
                    "think_video": "What did you think of this video? | How is this recommended content?",
                    "try": "Try",
                    ...
                    "recommend_subscribed_lable_tips": "Only show subscribed recommendations",
                    "title_add_shorts_upload_date": "Add Shorts upload time",
                    "title_shorts_change_author_name": "Change Shorts username to channel name",
                    "config_info": "Config info",
                    "page_info": "Page info",
                    "rule_info": "Rule info",
                    "del_config_confirm_tips": "Are you sure you want to delete all configuration settings?",
                    "btn_dbclick_download_video_tips": "Double-click the like button to download the video, double-click the dislike button to download the audio",
                    ...
                    "btn_sponsorblock_tips": "Automatically skip sponsor segments using SponsorBlock API"
                }
            },
        ...
        const interval_id = setInterval(() => {
            ...
            if (search_input_node) {
                ...
                const oninput = function (event) {
                    if ([display_error_keyword, open_config_keyword, reset_config_keyword, '2666'].includes(this.value)) {
                        setTimeout(() => {
    ...
    function hide_create_button() {
        ...
        const selectorParts = labels.map(l => `ytd-button-renderer.ytd-masthead button[aria-label="${l}"]`);
...
function hide_explore_more_topics_section() {
    ...
    for (const section of sections) {
        let titleSpan =
            section.querySelector('yt-shelf-header-layout h2.yt-shelf-header-layout__title span.yt-core-attributed-string') ||
            section.querySelector('.yt-shelf-header-layout__title span.yt-core-attributed-string') ||
    ...
    function hide_shorts_sections_if_disabled() {
        ...
        $$('#contents .yt-shelf-header-layout__title .yt-core-attributed-string').forEach(span => {
        ...
        // Mobile / grid-shelf based "Shorts" shelves (your grid-shelf-view-model example)
...
`;
        ...
        win_config.recommend_btn.forEach(recommend_item_info => {
            if (recommend_item_info.split_line) {
                ...
                p.innerText = flag_info[recommend_item_info.title] || recommend_item_info.title;
                item_groups.push(p);
                return;
            }
            const recommend_id = recommend_item_info.id;
            const recommend_title = flag_info[recommend_item_info.title] || recommend_item_info.title;
        ...
        function remove_popup_hander(event) {
            if ((event && event.target && !popup.contains(event.target)) || (event && event.target === closeButton) || event === 'rm') {
                popup.remove();
            ...
            if (path_info.express.includes('UPCOMING')) {
                try {
                    ...
                    if (user_data.channel_infos.ids.includes(id)) {
                        ...
                        log('Do not filter ' + name + (path_info.condition_value === 'UPCOMING' ? ' upcoming Live' : ' ongoing Live'), 'shorts');
                        return true;
                    }
                    let msg = `Filtering ${id} ${path_info.condition_value === 'UPCOMING' ? ' upcoming Live' : ' ongoing Live'}`;
                    log(msg, 'shorts');
                } catch (error) {
                    log(error, -1);
                }
                    ...
                    if (["mobile_yt_home", "mobile_yt_watch"].includes(page_type)) {
                        tmp_item = {
                            "shortsLockupViewModel": {
                                ...
                                "onTap": {
                                    "innertubeCommand": {
                                        "clickTrackingParams": "CK8BEIf2BBgAIhMIqeqAyo7QhQMVz3lMCB2mCA0JWg9GRXdoYXRfdG9fdG9wYXRjaJoBBQgkEI4e",
                                        "commandMetadata": {
                                            "webCommandMetadata": {
                                                "url": "/shorts/" + id,
                                                "webPageType": "WEB_PAGE_TYPE_SHORTS",
                ...
                this.get_shorts_info(id).then((author_info) => {
                    ...
                    if (author_id && user_data.channel_infos.ids.includes(author_id)) {
                        if (user_data.shorts_list.some((value) => { return value.id === id; })) {
                            log('Already exists from ' + author_name + ': ' + title, 'shorts');
                        } else {
                            log('Not filtering ' + author_name + "'s short: " + title, 'shorts');
                            const shorts_info = {
                                id: id,
                                title: title,
                                author_id: author_id,
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function (response) {
                        ...
                        try {
                            ...
                            log('Fetched subscription list successfully: ' + user_data.channel_infos.ids.length + ' channels', 0);
                        } catch (error) {
                            if (retry < 3) {
                    ...
                    onload: function (response) {
                        ...
                        if (match && match.length > 1) {
                            ...
                            if (retry < 3) {
                                ...
                                log('Failed to get channel_id', response, response.responseText, -1);
                            }
                        }
                    },
                    onerror: function (error) {
        ...
        return {
            ...
            update(tmp_user_data) {
                ...
                if (!tmp_user_data.watch_page_config) {
                    ...
                    if (tmp_user_data.watch_page_config.hide_live_chat_replay === undefined) {
        ...
        class DATA_PROCESS {
            ...
            obj_process(json_obj, express_list, traverse_all = false) {
                ...
                function add_data_to_abs_path(params) {
                    ...
                    if (array_index === '*') {
                        ...
                        try {
                            ...
                            log('obj_process failed to get array length --->' + path, error, -1);
                            return;
                        }
                        ...
                        if (index_arr.length !== 2) return log('obj_process array index format error --->' + path, -1);
                ...
                function obj_modify(json_obj, path_info) {
                    ...
                    if (!last_obj) {
                        debugger;
                        return log('obj_modify failed, object not found --->' + path_info, -1);
                    }
                    if (operator === '=-') {
                ...
                for (let condition_list of Object.values(condition_infos)) {
                    ...
                    for (let condition of condition_list) {
                        ...
                        if (result) {
                            ...
                            log('Condition satisfied -->', condition, typeof condition_value === 'object' ? '[object Object]' : condition_value, 'obj_process');
                            break;
                        }
                    }
    ...
    function sb_getVideoId() {
        try {
            ...
            const meta = unsafeWindow.document.querySelector('meta[itemprop="videoId"]');
    ...
    function init_sponsorblock() {
        ...
        if (!['yt_watch', 'mobile_yt_watch', 'yt_shorts', 'mobile_yt_shorts', 'yt_music_watch'].includes(page_type)) {
            return;
        }
    ...
    function init_create_button_observer() {
        const header = unsafeWindow.document.querySelector('ytd-masthead') || unsafeWindow.document.querySelector('ytm-app > ytm-mobile-topbar-renderer');
        if (!header) return;
    ...
    function init_global_shorts_blocker() {
        ...
        function blockShorts() {
            ...
            // Links to /shorts/ inside regular grids
            ...
            // Fallback: any rich section whose header span contains "Shorts"
        ...
        // Expose for debugging / manual trigger if needed
    ...
    function apply_hide_buttons_css() {
        ...
        if (user_data.hide_all_buttons === 'on') {
            ...
            if (user_data.hide_ask_button === 'on') {
                ...
                rules.push('button .yt-spec-button-shape-next__button-text-content:has(> span, > div, > *) { }');
            }

            if (user_data.hide_download_button === 'on') {
                rules.push('ytd-download-button-renderer { display: none !important; }');
                rules.push('button[aria-label="Download"] { display: none !important; }');
            }

            if (user_data.hide_share_button === 'on') {
                rules.push('#actions #menu ytd-menu-renderer > yt-button-view-model { display: none !important; }');
            ...
            if (user_data.hide_more_actions_button === 'on') {
                ...
                rules.push('button[aria-label="More actions"] { display: none !important; }');
            }

            if (user_data.hide_save_button === 'on') {
                rules.push('button[aria-label="Save to playlist"] { display: none !important; }');
            }
            ...
            if (user_data.hide_like_bar === 'on') {
                rules.push('.ytSegmentedLikeDislikeButtonViewModelSegmentedButtonsWrapper { display: none !important; }');
            }
    ...
    function display_hide_buttons_win() {
        const existing = unsafeWindow.document.getElementById('yt-hide-buttons-popup');
...
`;
        ...
        checkboxById['hb_livechat_replay'].addEventListener('change', () => {
            user_data.watch_page_config.hide_live_chat_replay =
                checkboxById['hb_livechat_replay'].checked ? 'on' : 'off';
            user_data_api.set();
            if (user_data.watch_page_config.hide_live_chat_replay === 'on') {
