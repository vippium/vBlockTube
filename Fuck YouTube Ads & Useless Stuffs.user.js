...
// @version      1.0
// @description  Very Useful thing for me (Not for M*therF@ckers)
// @author       vippium
...
function video_download(url = href, isAudioOnly = false, targetNode = null) {
    ...
    // log('Download URL', url, isAudioOnly, 0);
...
function get_config_api() {
        ...
        flag_infos: {
            ...
            get_rules: function (page_type_, type) {
                ...
                if (page_type_.includes('yt_shorts')) {
                    ...
                    // Add upload time // Change channel name
                ...
                if (page_type_.includes('yt_watch')) {
                    return function (json_obj) {
                        ...
                        videoRenderer_path
                            ...
                            mobile
                                /.itemSectionRenderer.contents[0].videoWithContextRenderer
                        section_path
                            desktop
                                /.reelShelfRenderer
                            mobile
                        ...
                        */
                        ...
                        // Youtuber merchandise ads https://www.youtube.com/watch?v=dFjXK8xpurY&list=RDCLAK5uy_mijutvVbzp7bbNlWt-B5U90qb5KplCkSQ&index=3
                        ...
                        // Recommendation sections that may appear under the video
    ...
    function display_error_win(msg) {
        ...
        container.innerHTML = `
        <h2 class="recommend-title" style="user-select:none;font-weight:bold;font-size:large;background-color:#3498db;color:#ffffff;border:none;padding:5px 10px;border-radius:5px;text-align:center;margin:0 auto;width:100px;">Information (message)</h2>
        ...
        <div style="display:flex;justify-content:space-around;margin-top:10px;">
            <button class="btn" id="copyBtn" style="cursor:pointer;background-color:#3498db;color:#ffffff;border:none;padding:5px 10px;border-radius:5px;">Copy</button>
            <button class="btn" id="closeBtn2" style="cursor:pointer;background-color:#3498db;color:#ffffff;border:none;padding:5px 10px;border-radius:5px;">Close</button>
        </div>`;
    ...
    function display_config_win() {
        ...
        return;
        // Hide search suggestion elements
        let count = 0;
        const interval_id = setInterval(() => {
            if (count = 30)
                clearInterval(interval_id);
    ...
    function check_update() {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (response) {
                ...
                // Extract version and metadata information from the online script
    ...
    function get_data_process() {
        class DATA_PROCESS {
            ...
            value_parse(parse_value, path_info = null, json_obj = null) {
                ...
                if (method_match) {
                    // When eval is restricted you can use num() and obj() to inject numeric/object args;
                    // those helper methods must be available on unsafeWindow in that mode, e.g. method(b("123",num(23)))
                    // When eval is not restricted you can call methods directly, e.g. method(b("123",23))
                    if (this.limit_eval) {
            ...
            paths_sort(paths_arr, key_name = null, reverse = false) {
                // Path format example: json_obj["onResponseReceivedActions"][0]["appendContinuationItemsAction"]
                // Supports sorting when array elements are objects (by a property inside them)
                // Supports sorting when array elements are strings (by the string value)
            ...
            obj_conditional(express_info, json_obj) {
                // json_obj is available directly inside eval
                ...
                for (let condition_list of Object.values(condition_infos)) {
                    ...
                    for (let condition of condition_list) {
                        ...
                        if (['child', 'parent'].includes(mod)) {
                            // child: /.a.b.c  -> path is relative to express_info.path
                            // parent: ..a.b.c -> path is relative to express_info.path
                            condition_path = this.get_relative_path(express_info.path, condition_path);
                        }
                        if (mod === 'other') {
                            // absolute path inside json_obj
                            condition_path = this.get_relative_path('json_obj', '/.' + condition_path);
                        }
                        if (mod === 'global') {
                            // extract data from global scope
