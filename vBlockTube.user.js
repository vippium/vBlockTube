...
`).trim().replace("PLACEHOLDER_COLOR", d ? "rgba(255,255,255,0.102)" : "rgba(0,0,0,0.051)");
  ...
  function init_custom_download_panel() {
    // Controlled by the top-level  custom_downloader  constant.
    if (!custom_downloader) return;

    // Detach any previous listener on re-init
    if (unsafeWindow.__vbt_dl_cleanup) {
      unsafeWindow.__vbt_dl_cleanup();
    }

    const STYLE_ID = "vbt-dl-style";
    if (!unsafeWindow.document.getElementById(STYLE_ID)) {
      const style = unsafeWindow.document.createElement("style");
      style.id = STYLE_ID;
      style.textContent = `
        #vbt-dl-popup {
        ...
        #vbt-dl-close:hover  { background-color: rgba(231,76,60,0.9) !important; }
        #vbt-dl-close:active { background-color: #c0392b !important; }
        #vbt-dl-tabs {
          display: flex;
        ...
        .vbt-dl-tab.active { color: #3498db; border-bottom-color: #3498db; }
        .vbt-dl-tab:hover:not(.active) { color: #444; }
        #vbt-dl-grid {
          display: grid;
        ...
        .vbt-dl-qbtn:hover:not(.selected) { border-color: #3498db; color: #3498db; background: #edf6ff; }
        .vbt-dl-qbtn.selected { border-color: #3498db; background: #3498db; color: #fff; }
        #vbt-dl-bottom {
          padding: 0 10px 10px;
          display: flex;
          flex-direction: column;
        ...
        #vbt-dl-btn:hover:not(:disabled) { background-color: #2980b9 !important; }
        #vbt-dl-btn:disabled { opacity: 0.55; cursor: not-allowed; }
        #vbt-dl-progress-wrap { display: none; flex-direction: column; gap: 4px; }
        #vbt-dl-progress-wrap.visible { display: flex; }
        #vbt-dl-progress-track {
          height: 5px;
          background: #e0e0e0;
          border-radius: 3px;
          overflow: hidden;
        }
        #vbt-dl-progress-fill {
          height: 100%;
          width: 0%;
          background: #3498db;
          border-radius: 3px;
        ...
        #vbt-dl-progress-text { font-size: 11px; color: #888; text-align: center; }
        #vbt-dl-status { font-size: 12px; color: #666; text-align: center; min-height: 15px; }
        #vbt-dl-status.error   { color: #c0392b; }
        #vbt-dl-status.success { color: #27ae60; }
        html[dark] #vbt-dl-popup,
        html:not([dark]):not([light]) #vbt-dl-popup {
          background-color: #1e1e1e !important;
          border-color: #3498db !important;
        }
        html[dark] #vbt-dl-tabs,
        html:not([dark]):not([light]) #vbt-dl-tabs {
          border-bottom-color: #404040 !important;
        }
        html[dark] .vbt-dl-tab:not(.active),
        html:not([dark]):not([light]) .vbt-dl-tab:not(.active) { color: #888 !important; }
        html[dark] .vbt-dl-tab:hover:not(.active),
        html:not([dark]):not([light]) .vbt-dl-tab:hover:not(.active) { color: #ccc !important; }
        html[dark] .vbt-dl-qbtn:not(.selected),
        html:not([dark]):not([light]) .vbt-dl-qbtn:not(.selected) {
          background: #2d2d2d !important;
          border-color: #404040 !important;
          color: #ccc !important;
        }
        html[dark] .vbt-dl-qbtn:hover:not(.selected),
        html:not([dark]):not([light]) .vbt-dl-qbtn:hover:not(.selected) {
          border-color: #3498db !important;
          color: #3498db !important;
          background: #1a2a36 !important;
        }
        html[dark] #vbt-dl-progress-track,
        html:not([dark]):not([light]) #vbt-dl-progress-track { background: #404040 !important; }
        html[dark] #vbt-dl-status,
        html:not([dark]):not([light]) #vbt-dl-status { color: #aaa !important; }
        html[dark] #vbt-dl-progress-text,
        html:not([dark]):not([light]) #vbt-dl-progress-text { color: #888 !important; }
      `;
      unsafeWindow.document.head.appendChild(style);
    }
    ...
    function _getVideoId() {
      return new URLSearchParams(unsafeWindow.location.search).get("v") || null;
    }
    function _getVideoUrl() {
      const id = _getVideoId();
      return id ? `https://www.youtube.com/watch?v=${id}` : unsafeWindow.location.href;
    }
    function _getVideoTitle() {
      try {
        const el =
          unsafeWindow.document.querySelector("h1.ytd-video-primary-info-renderer yt-formatted-string") ||
          unsafeWindow.document.querySelector("ytd-watch-metadata h1 yt-formatted-string") ||
          unsafeWindow.document.querySelector("h1.title");
        return el ? el.textContent.trim() : "video";
      } catch { return "video"; }
    }
    function _sanitize(n) {
    ...
    function buildPanel() {
      ...
      let selectedFmt  = "video";
      let selectedQual = "1080";
      let selectedBr   = "320";

      const popup = doc.createElement("div");
      popup.id = "vbt-dl-popup";

      const header = doc.createElement("div");
      header.id = "vbt-dl-header";
      header.textContent = "Download Video";

      const closeBtn = doc.createElement("button");
      closeBtn.id = "vbt-dl-close";
      closeBtn.innerHTML = "\xd7";
      closeBtn.title = "Close";
      header.appendChild(closeBtn);

      const tabBar = doc.createElement("div");
      tabBar.id = "vbt-dl-tabs";
      const tabVideo = doc.createElement("button");
      tabVideo.className = "vbt-dl-tab active";
      tabVideo.textContent = "Video";
      const tabAudio = doc.createElement("button");
      tabAudio.className = "vbt-dl-tab";
      tabAudio.textContent = "Audio";
      tabBar.append(tabVideo, tabAudio);

      const grid = doc.createElement("div");
      grid.id = "vbt-dl-grid";

      function renderGrid(items, selected, onSelect) {
        grid.innerHTML = "";
        items.forEach(val => {
          const btn = doc.createElement("button");
          btn.className = "vbt-dl-qbtn" + (val === selected ? " selected" : "");
          btn.textContent = val === "2160" ? "4K" : val === "1440" ? "1440p"
            : VIDEO_QUALITIES.includes(val) ? val + "p" : val + " kbps";
          btn.addEventListener("click", () => {
            grid.querySelectorAll(".vbt-dl-qbtn").forEach(b => b.classList.remove("selected"));
      ...
      renderGrid(VIDEO_QUALITIES.slice().reverse(), selectedQual, v => { selectedQual = v; });
      ...
      tabAudio.addEventListener("click", () => {
        ...
        renderGrid(AUDIO_BITRATES.slice().reverse(), selectedBr, v => { selectedBr = v; });
      });

      const bottom = doc.createElement("div");
      bottom.id = "vbt-dl-bottom";

      const progressWrap = doc.createElement("div");
      progressWrap.id = "vbt-dl-progress-wrap";
      const progressTrack = doc.createElement("div");
      progressTrack.id = "vbt-dl-progress-track";
      const progressFill = doc.createElement("div");
      progressFill.id = "vbt-dl-progress-fill";
      progressTrack.appendChild(progressFill);
      const progressText = doc.createElement("div");
      progressText.id = "vbt-dl-progress-text";
      progressWrap.append(progressTrack, progressText);

      const statusEl = doc.createElement("div");
      statusEl.id = "vbt-dl-status";

      const dlBtn = doc.createElement("button");
      dlBtn.id = "vbt-dl-btn";
      dlBtn.textContent = "Download";

      bottom.append(progressWrap, dlBtn, statusEl);
      popup.append(header, tabBar, grid, bottom);
      ...
      dlBtn.addEventListener("click", async () => {
        ...
        if (!videoId) {
          ...
          statusEl.textContent = "No video ID \u2014 open a YouTube watch page first.";
          return;
        }
        ...
        progressText.textContent = "Fetching session key\u2026";

        try {
          const keyResp = await new Promise((res, rej) => {
            GM_xmlhttpRequest({
              method: "GET", url: API.KEY_URL, headers: DL_HEADERS,
              onload: res,
          ...
          if (keyResp.status !== 200)
            throw new Error(`Key request failed (HTTP ${keyResp.status})`);
          const keyData = JSON.parse(keyResp.responseText);
          if (!keyData?.key) throw new Error("Unexpected API response (no key)");

          progressFill.style.width = "15%";
          progressText.textContent = "Requesting conversion\u2026";

          const videoUrl = _getVideoUrl();
          let payload;
          if (selectedFmt === "video") {
            payload = {
              link: videoUrl, format: "mp4", audioBitrate: "128",
              videoQuality: selectedQual, filenameStyle: "pretty",
              vCodec: parseInt(selectedQual, 10) > 1080 ? "vp9" : "h264",
            };
          } else {
            payload = {
              link: videoUrl, format: "mp3",
              audioBitrate: selectedBr, filenameStyle: "pretty",
            };
          }

          const convResp = await new Promise((res, rej) => {
            GM_xmlhttpRequest({
              method: "POST", url: API.CONVERT_URL,
              headers: { ...DL_HEADERS, key: keyData.key },
              data: JSON.stringify(payload),
              onload: res,
              onerror:   () => rej(new Error("Network error")),
              ontimeout: () => rej(new Error("Conversion timeout")),
            });
          });
          if (convResp.status !== 200)
            throw new Error(`Conversion failed (HTTP ${convResp.status})`);
          const convData = JSON.parse(convResp.responseText);
          if (!convData?.url)
            throw new Error(convData?.error || "No download URL \u2014 video may be restricted or private");
          ...
          await new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
              method: "GET", url: convData.url, responseType: "blob",
              headers: {
                "User-Agent": DL_HEADERS["User-Agent"],
              ...
              onprogress: (prog) => {
                if (prog.total > 0) {
                  ...
                  progressFill.style.width = `${35 + pct * 0.65}%`;
                  progressText.textContent =
                    `${_fmtBytes(prog.loaded)} / ${_fmtBytes(prog.total)} (${pct}%)`;
                }
              },
              onload: (resp) => {
                if (resp.status !== 200 || !resp.response)
                  return reject(new Error(`File fetch failed (HTTP ${resp.status})`));
                const blob = resp.response;
                if (blob.size === 0) return reject(new Error("Downloaded file is 0 bytes"));
                progressFill.style.width = "100%";
                progressText.textContent = `Complete \u2014 ${_fmtBytes(blob.size)}`;
                const ext = selectedFmt === "video" ? "mp4" : "mp3";
                const filename = _sanitize(convData.filename || `${_getVideoTitle()}.${ext}`);
              ...
              ontimeout: () => reject(new Error("File download timed out")),
            });
          });

          statusEl.className = "success";
          statusEl.textContent = "\u2713 Download started \u2014 check your downloads bar";
    ...
    function captureHandler(e) {
      if (!unsafeWindow.location.pathname.startsWith("/watch")) return;
      const btn = e.target.closest('button[aria-label="Download"]');
      if (!btn) return;
      e.stopImmediatePropagation();
      e.preventDefault();
      // Second click on Download button closes the panel (toggle)
      const existing = unsafeWindow.document.getElementById("vbt-dl-popup");
      if (existing) { existing.remove(); return; }
      unsafeWindow.document.body.appendChild(buildPanel());
    }

    unsafeWindow.document.addEventListener("click", captureHandler, true);

    unsafeWindow.__vbt_dl_cleanup = () => {
      unsafeWindow.document.removeEventListener("click", captureHandler, true);
      const old = unsafeWindow.document.getElementById("vbt-dl-popup");
      if (old) old.remove();
      delete unsafeWindow.__vbt_dl_cleanup;
    };
  }
